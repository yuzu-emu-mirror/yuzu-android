...
Vic::Vic(Host1x& host1x_, s32 id_, Nvdec& nvdec_processor_, u32 syncpt)
    : CDmaPusher{host1x_, id_},
      nvdec_processor{nvdec_processor_}, id{id_}, syncpoint{syncpt}, has_sse41{HasSSE41()} {
...
void Vic::ProcessMethod(u32 method, u32 arg) {
    ...
    case Method::Execute: {
        auto vic_syncpoint = host1x.GetSyncpointManager().GetGuestSyncpointValue(syncpoint);
        auto nvdec_syncpoint =
            host1x.GetSyncpointManager().GetGuestSyncpointValue(nvdec_processor.GetSyncpoint());
        if (vic_syncpoint + 4 < nvdec_syncpoint) {
            nvdec_processor.SetWait();
        }
        Execute();
    } break;
    default:
...
void Vic::Execute() {
    ...
    if (Settings::values.nvdec_emulation.GetValue() == Settings::NvdecEmulation::Off) [[unlikely]] {
        ...
        for (size_t i = 0; i < config.slot_structs.size(); i++) {
            ...
            auto frame = nvdec_processor.GetFrame(luma_offset);
            if (!frame.get()) {
